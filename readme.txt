...
특히 코드페이지 949 가 지원 가능해야 합니다. 하지만 Warp 4 FixPak #4
또는 #5 부터 형식적으로 모든 코드페이지가 지원됩니다.
...
현재 SBCS OS/2 에서는 글꼴 연관 기능이 제공되고 있지 않습니다. 그래서
기본적으로 설정되어 있는 글꼴이 한글 글꼴을 포함하고 있지 않게 되면,
한글을 출력할 수 없습니다. 따라서 글꼴 팔레트를 이용해서 해당 윈도의
글꼴을 한글 글꼴이 포함된 글꼴로 바꾸어 주셔야 합니다.
...
6. 제한 사항 또는 알려진 벌레
  ...
  - Task List(창목록)에서 KIME 을 활성화 시킬 경우, Task List 윈도가 사라지지 
    않습니다.
    이것은 KIME 상태 상자를 Frame Window 를 쓰지 않고, 일반 윈도를 쓴 탓에 태
    스크 스위칭에 관계된 메세지를 제대로 처리하지 못해서 생겨나는 현상인 것 
    같습니다.

  - Task List 에서 KIME 을 활성화 시킬 때, KIME 이 다른 윈도에 가려져 있으면
    KIME 의 Z-order 가 바뀌지 않습니다. 따라서 KIME 상태창이 안보입니다.
  ...
  - 프로파일 파일과 예외 목록 파일은 현재 작업 디렉토리를 기준으로 처리
    합니다.
...
7. 할 것들
  ...
  - 한국어판 OS/2 의 기본 IME 와 연동, Shift-Space 변환따위
